...
def mandelbrot_numba_parallel(size, iterations):
    """Calcolo Mandelbrot con NUMBA parallelo tramite prange."""
    image = np.zeros((size, size), dtype=np.int32)
    # prange enables automatic loop parallelization
...
def plot_mandelbrot(image, size, iterations,cmap):
    ...
    plt.tight_layout()
    plt.savefig("mandelbrot_set.svg")
    plt.show()
...
def main():
    ...
    image = mandelbrot_numba_parallel(SIZE, ITERATIONS)
