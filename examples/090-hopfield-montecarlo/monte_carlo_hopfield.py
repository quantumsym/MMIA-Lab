...
def plot_pattern_recovery(pattern, recovered, output_file='pattern_recovery.svg'):
    ...
    # Reshape for visualization (assuming square patterns)
...
def demonstrate_monte_carlo_hopfield():
    ...
    # Plot energy evolution from one of the runs
    plot_energy_evolution(results_moderate['energy_history'], 
                         output_file='mc_energy_evolution.svg')
    
    # Plot temperature comparison
    temp_comparison = {
        0.1: {'energy_history': results_low['energy_history'][:500],
              'acceptance_history': [results_low['acceptance_rate']]*500},
        0.5: {'energy_history': results_moderate['energy_history'][:500],
              'acceptance_history': [results_moderate['acceptance_rate']]*500},
        2.0: {'energy_history': results_high['energy_history'][:500],
              'acceptance_history': [results_high['acceptance_rate']]*500}
    }
    plot_temperature_comparison(temp_comparison, output_file='temperature_comparison.svg')
    
    # Plot pattern recovery (using first pattern)
    if len(patterns) > 0:
        plot_pattern_recovery(patterns[0], results_moderate['final_state'], 
