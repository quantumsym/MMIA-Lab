...
def plot_tsp_tour(cities, tour, title='TSP Tour', output_file='tsp_tour.svg'):
    """
    Plots a TSP tour with cities and connections.
    
    Args:
        cities (np.ndarray): Array of city coordinates (N x 2).
    ...
    # Plot cities
    plt.scatter(cities[:, 0], cities[:, 1], c='red', s=200, zorder=3, edgecolors='black', linewidths=2)
    
    # Plot tour connections
    for i in range(len(tour)):
        city_a = tour[i]
        city_b = tour[(i + 1) % len(tour)]
        plt.plot([cities[city_a, 0], cities[city_b, 0]], 
    ...
    plt.show()
    print(f"Saved TSP tour plot to {output_file}")

def plot_tsp_energy_evolution(energy_history, output_file='tsp_energy_evolution.svg'):
    """
    Plots energy evolution during TSP optimization.
    
    Args:
        energy_history (list): List of energy values over optimization steps.
        output_file (str): Output SVG file name.
    """
    plt.figure(figsize=(12, 6))
    plt.plot(energy_history, 'g-', linewidth=2, alpha=0.8)
    plt.xlabel('Optimization Step', fontsize=12)
    plt.ylabel('Energy E', fontsize=12)
    plt.title('TSP Energy Evolution', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_file, format='svg', dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved TSP energy evolution plot to {output_file}")

def plot_tsp_comparison(cities, optimal_tour, sa_tour, pt_tour, output_file='tsp_comparison.svg'):
    """
    Compares optimal, SA, and PT solutions for TSP.
    
    Args:
        cities (np.ndarray): Array of city coordinates.
        optimal_tour (list): Optimal tour.
        sa_tour (list): SA solution tour.
        pt_tour (list): PT solution tour.
        output_file (str): Output SVG file name.
    """
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    
    tours = [optimal_tour, sa_tour, pt_tour]
    titles = ['Optimal Tour', 'Simulated Annealing', 'Parallel Tempering']
    colors = ['green', 'blue', 'red']
    
    for ax, tour, title, color in zip(axes, tours, titles, colors):
        # Plot cities
        ax.scatter(cities[:, 0], cities[:, 1], c='red', s=150, zorder=3, edgecolors='black', linewidths=1.5)
        
        # Plot tour
        for i in range(len(tour)):
            city_a = tour[i]
            city_b = tour[(i + 1) % len(tour)]
            ax.plot([cities[city_a, 0], cities[city_b, 0]], 
                   [cities[city_a, 1], cities[city_b, 1]], 
                   color=color, linewidth=2, alpha=0.7)
        
        # Label cities
        for i, (x, y) in enumerate(cities):
            ax.text(x, y, str(i), fontsize=10, ha='center', va='center', color='white', fontweight='bold')
    ...
    plt.savefig(output_file, format='svg', dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved TSP comparison plot to {output_file}")

def plot_tsp_tour_length_comparison(tour_lengths, method_names, output_file='tsp_tour_length_comparison.svg'):
    """
    Bar plot comparing tour lengths from different methods.
    
    Args:
        tour_lengths (list): List of tour lengths.
        method_names (list): List of method names.
        output_file (str): Output SVG file name.
    """
    plt.figure(figsize=(10, 6))
    colors = ['green', 'blue', 'red', 'orange'][:len(method_names)]
    
    plt.bar(method_names, tour_lengths, color=colors, edgecolor='black', linewidth=2, alpha=0.7)
    plt.ylabel('Tour Length', fontsize=12)
    plt.title('TSP Tour Length Comparison', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3, axis='y')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(output_file, format='svg', dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved TSP tour length comparison plot to {output_file}")
...
def demonstrate_tsp_optimization():
    ...
    # Plot optimal tour (from small_cities test)
    plot_tsp_tour(small_cities, optimal_tour, title='Optimal TSP Tour (6 Cities)',
                 output_file='tsp_tour_optimal.svg')
    
    # Plot SA tour (from small_cities test)
    plot_tsp_tour(small_cities, sa_result['best_tour'], title='TSP Tour - Simulated Annealing (6 Cities)',
                 output_file='tsp_tour_sa.svg')
    
    # Plot PT tour (from small_cities test)
    plot_tsp_tour(small_cities, pt_result['best_tour'], title='TSP Tour - Parallel Tempering (6 Cities)',
                 output_file='tsp_tour_pt.svg')
    
    # Plot energy evolution from large SA
    if 'energy_history' in large_sa:
        plot_tsp_energy_evolution(large_sa['energy_history'], 
                                 output_file='tsp_energy_evolution_sa.svg')
    
    # Plot comparison of all three tours (small instance)
    plot_tsp_comparison(small_cities, optimal_tour, sa_result['best_tour'], pt_result['best_tour'],
                       output_file='tsp_comparison.svg')
    
    # Plot tour length comparison (small instance)
    tour_lengths = [optimal_length, sa_result['best_tour_length'], pt_result['best_tour_length']]
    method_names = ['Optimal', 'SA', 'PT']
    plot_tsp_tour_length_comparison(tour_lengths, method_names,
                                   output_file='tsp_tour_length_comparison.svg')
