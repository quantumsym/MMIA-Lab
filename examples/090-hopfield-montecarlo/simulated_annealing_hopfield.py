...
def plot_sa_energy_evolution(sa_results, output_file='sa_energy_evolution.svg'):
    ...
    ax2 = ax1.twinx()
    ax2.plot(sa_results['temperature_history'], 'r--', linewidth=2, alpha=0.7, label='Temperature')
...
def demonstrate_advanced_optimization():
    ...
    schedule_data = {
        'temperatures': sa_results['temperature_history'],
    ...
    plot_sa_energy_evolution(sa_results, output_file='sa_energy_evolution.svg')
    
    # Plot PT replica energies
    plot_pt_replica_energies(pt_results, output_file='pt_replica_energies.svg')
    
    # Plot PT exchange matrix (if available)
    if 'exchange_matrix' in pt_results:
        plot_pt_exchange_matrix(pt_results['exchange_matrix'], 
                               pt_results['temperatures'],
                               output_file='pt_exchange_matrix.svg')
    
    # Plot SA vs PT comparison
    plot_sa_vs_pt_comparison(sa_results, pt_results, output_file='sa_vs_pt_comparison.svg')
